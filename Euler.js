// utility functions
function isDivisible(num, divisor){
    return num % divisor == 0;
}
function fib(n){
  if (n<= 1) {
    return 1;
  } else {
    return fib(n-1) + fib (n-2);
  }
}

function primeFactors(num){
  var factors = [];
  for (var i = 2; i <= num; i++) {
    while(isDivisible(num, i)){
    factors.push(i);

    num =num / i;

  }
  }
  return factors;
}
function regFactors(num){
  var factors = [];
  for (var i = 1; i <= num; i++) {
    if(isDivisible(num, i)){
    factors.push(i);
  }
  }
  return factors.length;
}
function palindromify(num){

  var pal = JSON.stringify(num);
  pal = pal.split("");
  pal.reverse();
  pal = pal.join("");
  pal = Number(pal);
  return pal;

}
function getSum(total, num) {
return total + num;
}
function isPrime(numm){
   if(numm <=1) {
     return false;
   }
  if(numm == 2){
    return true;
  }
  if(numm % 2 == 0){
     return false;
     }

  var counter = 3;

  while ((counter * counter) <= numm){
    if(numm % counter == 0){
       return false;
       }else{
         counter += 2;
       }
  }
    return true;
}
  function nextTri() {
    var currentTri = 0;
  var currentTriNum = 0;
      currentTri += ++currentTriNum;
      return currentTri;
  }





//ProblemTracker
var problemTracker = [];

//all problems are solved below
// some data can be found in the data div
problemTracker[13] = function (){

}
problemTracker[12] = function (){
  return "skipped";
}

problemTracker[11] = function (data){

  var numGrid =
"08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 \
49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00 \
81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65 \
52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91 \
22 31 16 71 51 67 63 89 41 92 36 54 22 40 40 28 66 33 13 80 \
24 47 32 60 99 03 45 02 44 75 33 53 78 36 84 20 35 17 12 50 \
32 98 81 28 64 23 67 10 26 38 40 67 59 54 70 66 18 38 64 70 \
67 26 20 68 02 62 12 20 95 63 94 39 63 08 40 91 66 49 94 21 \
24 55 58 05 66 73 99 26 97 17 78 78 96 83 14 88 34 89 63 72 \
21 36 23 09 75 00 76 44 20 45 35 14 00 61 33 97 34 31 33 95 \
78 17 53 28 22 75 31 67 15 94 03 80 04 62 16 14 09 53 56 92 \
16 39 05 42 96 35 31 47 55 58 88 24 00 17 54 24 36 29 85 57 \
86 56 00 48 35 71 89 07 05 44 44 37 44 60 21 58 51 54 17 58 \
19 80 81 68 05 94 47 69 28 73 92 13 86 52 17 77 04 89 55 40 \
04 52 08 83 97 35 99 16 07 97 57 32 16 26 26 79 33 27 98 66 \
88 36 68 87 57 62 20 72 03 46 33 67 46 55 12 32 63 93 53 69 \
04 42 16 73 38 25 39 11 24 94 72 18 08 46 29 32 40 62 76 36 \
20 69 36 41 72 30 23 88 34 62 99 69 82 67 59 85 74 04 36 16 \
20 73 35 29 78 31 90 01 74 31 49 71 48 86 81 16 23 57 05 54 \
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48";
  data = data.trim();
  var rows = data.split('\n');
  var result = [];
  for (var i = 0; i < rows.length; i++) {
          result.push(rows[i].split(' '));
      }
  //console.log(result);

  var maxProduct = 0;
  //horizontal
  for (var i = 0; i < result.length - 4; i++) {
          for (var j = 0; j < result[i].length - 4; j++) {
              var product = result[i][j] *
                              result[i][j + 1] *
                              result[i][j + 2] *
                              result[i][j + 3];
              if (product > maxProduct) maxProduct = product;
          }
      }
      // vertical
      for (var j = 0; j < result[0].length - 4; j++) {
          for (var i = 0; i < result.length - 4; i++) {
              var product = result[i][j] *
                              result[i][j + 1] *
                              result[i][j + 2] *
                              result[i][j + 3];
              if (product > maxProduct) maxProduct = product;
          }
      }
      //diagonal1
      for (var j = 0; j < result[0].length - 4; j++) {
          for (var i = 0; i < result.length - 4; i++) {
              var product = result[i][j] *
                              result[i + 1][j + 1] *
                              result[i + 2][j + 2] *
                              result[i + 3][j + 3];
              if (product > maxProduct) maxProduct = product;
          }
      }
      //diagonal2
      for (var j = 0; j < result[0].length - 4; j++) {
          for (var i = 0; i < result.length - 4; i++) {
              var product = result[i + 3][j] *
                              result[i + 2][j + 1] *
                              result[i + 1][j + 2] *
                              result[i][j + 3];
              if (product > maxProduct) maxProduct = product;
          }
      }
      return maxProduct;
}
problemTracker[10] = function (){
  var Array = [2];
  for(var i = 1; i < 2000000; i += 2){
    if(isPrime(i)){
       Array.push(i);
       }
  }
  return Array.reduce(getSum);
}

problemTracker[9] = function (){
  var num =0;
  for(var a = 0; a<500; a++){
    for(var b = a+1; b<500; b++){
      for(var c = b+1; c<500; c++){
        if(a+b+c !== 1000){
           continue;
           }
           if((a*a) +(b*b) ===(c*c) ){
          return num += a*b*c;
        }

      }
    }
  }
}
problemTracker[8] = function (){
  var bigNum =
"73167176531330624919225119674426574742355349194934\
96983520312774506326239578318016984801869478851843\
85861560789112949495459501737958331952853208805511\
12540698747158523863050715693290963295227443043557\
66896648950445244523161731856403098711121722383113\
62229893423380308135336276614282806444486645238749\
30358907296290491560440772390713810515859307960866\
70172427121883998797908792274921901699720888093776\
65727333001053367881220235421809751254540594752243\
52584907711670556013604839586446706324415722155397\
53697817977846174064955149290862569321978468622482\
83972241375657056057490261407972968652414535100474\
82166370484403199890008895243450658541227588666881\
16427171479924442928230863465674813919123162824586\
17866458359124566529476545682848912883142607690042\
24219022671055626321111109370544217506941658960408\
07198403850962455444362981230987879927244284909188\
84580156166097919133875499200524063689912560717606\
05886116467109405077541002256983155200055935729725\
71636269561882670428252483600823257530420752963450";

  var bigArray = bigNum.split("");
  var biggestPro = 0;
  var currentPro = 0;
  for (var i = 0; i <= (bigArray.length - 1); i++){
    currentPro = 1;

    for(var j = 0; j <=12; j++){
      currentPro *= Number(bigArray[i+j]);

    }
    if(currentPro > biggestPro){
      biggestPro = currentPro;
    }
  }

  return biggestPro;
}
problemTracker[7] = function (){
  var numPrimes = 1;
  var numm = 1;

  while(numPrimes <= 10001){
    numm = numm + 2;
    if(isPrime(numm)){
       numPrimes++;
       }

  }

  return numm;

}

problemTracker[6] = function (){
  var sqSum = 0;
  var Sum = 0;
  for(var i = 1; i <= 100; i++){
    sqSum += i;
    Sum += (i*i);
                               }


  //console.log(Sum, (sqSum * sqSum));
  return (sqSum*sqSum) - Sum;
}

problemTracker[5] = function (){
let found = false;
let number = 1;
while (!found) {
  for (let i = 2; i < 20; i++) {
      if (isDivisible(number, i)) {
          continue;
      } else {
          number++;
          i = 2;
      }
  }
  found = true;
}
return number;

}



problemTracker[4] = function (){
for(var i= 999; i>=850; i--){
  for(var j = 999; j>= 850; j--){
   //var palArray = [];
    var proSum = i*j;
  //palArray.push(proSum);
    //console.log(palArray);

    var palSum = palindromify(proSum);
    //console.log(palSum);
    if(proSum === palSum){
       return palSum;
    }
  }



}


}


problemTracker[3] = function (){
var bigNumber = 600851475143;
var factors = primeFactors(bigNumber);
return factors[factors.length - 1];
}



problemTracker[2] = function (){
  var sum = 0;
  for(var i=0; i < 100000; i++){
    var f = fib(i);
    if(f>4000000){
      return sum;
    }
    if(isDivisible(f,2)){
      sum += f;
    }
  }
};


problemTracker[1] = function (){
  var sum = 0;
  for (var i = 0; i < 1000; i++){
    if (isDivisible(i, 3) || isDivisible(i, 5)){
      sum += i;
    }
  }
  return sum;
};


















var solutionDiv = document.getElementById('solutions');
 for (var i = 1; i < problemTracker.length; i++) {
     var newDiv = document.createElement('div');
     var newLabel = document.createElement('label');
     var newInput = document.createElement('input');
     var newButton = document.createElement('button');
     var newSpan = document.createElement('span');
     var dataDiv = document.getElementById('d' + i);
     var data = dataDiv ? dataDiv.textContent : null;
     newButton.onclick = (function (probNum, input, datas) {
         return function () {
             input.value = problemTracker[probNum](datas);
         }
     })(i, newInput, data);
     newButton.appendChild(document.createTextNode('solve'));
     newInput.readOnly = true;
     newLabel.appendChild(document.createTextNode('Solution ' + i));
     newLabel.appendChild(newInput);
     newDiv.appendChild(newLabel);
     if (problemTracker[i].skipped) {
         newDiv.appendChild(document.createTextNode('*skipped'));
     } else {
         newDiv.appendChild(newButton);
     }
     newDiv.appendChild(newSpan);
     solutionDiv.appendChild(newDiv);
 }
